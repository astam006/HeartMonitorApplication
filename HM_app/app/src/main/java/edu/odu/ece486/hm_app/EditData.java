...
/**
 * Created by Larry on 10/22/2014.
 */
...
public class EditData extends LinearLayout{
    ...
    public byte[] getData() {
        ...
        if (inHexMode()) {
            return HexAsciiConverter.hexToBytes(text);
        } else {
    ...
    public void setData(byte[] data) {
        String text;
        if (inHexMode()) {
            text = HexAsciiConverter.bytesToHex(data);
        } else {
            text = HexAsciiConverter.bytesToAsciiMaybe(data);
            if (text == null) {
                text = "";
            }
        }
